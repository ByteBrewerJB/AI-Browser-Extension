...
7. [ ] Chain-confirmatie koppelen aan parser en async step-runner voorbereiden.
   - **Prioritering** – Chain-confirmatie moet de DSL-parser gebruiken zodat `[[step.output]]`-tokens en placeholdervariabelen realtime ingevuld worden vóórdat een keten start. Dit verkleint fouten bij inline triggers en legt de basis voor de async step-runner (cascading updates + annulering). We behandelen eerst de confirmatiemodal (focushooks, validatie, parserbinding), daarna volgt de step-runner-integratie.
   - **Documentatie** – Roadmap (`docs/handbook/product-roadmap.md`) aangescherpt met parser→confirmatie afhankelijkheden en de regressiegids (`docs/handbook/manual-regression-checklist.md`) aangevuld met een tijdelijke opmerking dat `[[step.output]]`-invulling pas gecontroleerd wordt na oplevering van deze taak.
   - **QA-notes** – Voor oplevering: `npm run lint`, `npm run test`, `npm run build` op Node 20.19.0. Handmatig: launcher via `//` en `..` openen, keten selecteren met placeholders en `[[step.output]]`, bevestigen dat de modal variabelen en step-output toont en dat annuleren + herstart geen oude waarden behoudt. Tot de feature landt, noteer regressieresultaten als "pending" met verwijzing naar deze taak in het logboek.
