...
16. [x] Zijbalk pin/hide/collapse state implementeren met gedeelde Zustand-store en UI-koppelingen. _(afgerond 2025-10-16)_
   - **Prioritering** – `sidebarVisibilityStore` levert nu één bron voor pin-, hide- en collapse-voorkeuren die popup, opties en content tegelijk hydrateren. Pinned-secties verschijnen bovenaan, verborgen secties renderen niet en collapse-knoppen werken vanuit de content-header. Volgende iteratie: undo/redo-flows voor bulkselecties plus automatische herstelopties voor verborgen secties zodra de toegankelijkheidsaanpassingen in alle surfaces zijn gevalideerd.
   - **Documentatie** – Nieuwe modules `src/shared/state/sidebarVisibilityStore.ts`, `src/shared/types/sidebar.ts`, contentkoppelingen (`src/content/sidebar-host.ts`, `src/content/ui-root.tsx`), popupkaart `src/popup/features/sidebar/SidebarSection.tsx` en dashboardtabel `src/options/features/sidebar/SidebarPreferences.tsx` toegevoegd. Deze log, de roadmap (`docs/handbook/product-roadmap.md`), designnotitie (`docs/design/sidebar-pin-wireframes.md`) en regressiegids (`docs/handbook/manual-regression-checklist.md`) bijgewerkt met voorkeurflows, data-attributen en QA-stappen.
 - **QA-notes** – Geautomatiseerd: `npm run lint`, `npm run test`, `npm run build` (Node 20.19.0) + nieuwe regressietests `tests/shared/state/sidebarVisibilityStore.spec.ts` (pinnen/inklappen/verborgen-state) en `tests/content/sidebarHost.spec.ts` (shadow-host attributen). Handmatig: in popup en dashboard voorkeuren toggelen en verifiëren dat de content-zijbalk direct mee verandert, inclusief `data-ai-companion-pinned-count`/`hidden-count`; op beide domeinen secties verbergen, herladen en bevestigen dat status behouden blijft in LTR en RTL. Resultaten toevoegen aan regressiegids en logboek.

17. [x] Toegankelijkheidsreview voor pin-, hide- en collapse-bediening. _(afgerond 2025-10-17)_
   - **Prioritering** – Alle surfaces gebruiken nu expliciete aria-labels, beschrijvingen en region-rolmarkering zodat screenreaders onderscheid kunnen maken tussen sectieknoppen. Collapse-acties verwijzen naar de sectietitel en popup-toggles groeperen hun bediening met context. Volgende stap: undo/redo-flows combineren met deze toegankelijkheidslagen zonder dubbele aankondigingen.
   - **Documentatie** – Componentupdates in `src/content/ui-root.tsx` (aria-labelledby/region, dynamische collapse-labels) en `src/popup/features/sidebar/SidebarSection.tsx` (aria-labels, role="group", beschrijvingskoppeling) zijn aangevuld met vertalingen (`src/shared/i18n/locales/{en,nl}/common.json`). Dit retrofitlog, de roadmap (`docs/handbook/product-roadmap.md`), designnotitie (`docs/design/sidebar-pin-wireframes.md`) en regressiegids (`docs/handbook/manual-regression-checklist.md`) beschrijven nu de nieuwe toegankelijkheidscontroles.
   - **QA-notes** – Geautomatiseerd: `npm run lint`, `npm run test`, `npm run build` (Node 20.19.0). Handmatig: in Chrome DevTools → Accessibility tree controleren dat de collapse-knop "Collapse {section}"/"Expand {section}" aankondigt en `aria-expanded` wijzigt; in de popup met `Tab` door de "Sidebar layout"-kaart lopen en bevestigen dat elke knop de sectienaam in de screenreadernaam bevat, ook na taalwissel (EN/NL). Resultaten vastleggen in regressiegids + logboek.

18. [x] Undo/redo-flows voor bulk pin/hide-acties ontwerpen en implementeren. _(afgerond 2025-10-18)_
   - **Prioritering** – `sidebarVisibilityStore` bewaart nu een history-stack en stuurt toasts met undo/redo voor pin- en hide-acties. Popup, dashboard en content tonen dezelfde meldingen zodat bulkmutaties (bijv. hide → collapse + unpin) als één herstelbare bewerking gelden. Volgende stap: automatische herstelopties voor langdurig verborgen secties onderzoeken.
   - **Documentatie** – Nieuwe toastcomponent `SidebarVisibilityToast` (popup/options/content), history-API in `src/shared/state/sidebarVisibilityStore.ts`, vertalingen (`src/shared/i18n/locales/{en,nl}/common.json`) en tests (`tests/shared/state/sidebarVisibilityStore.spec.ts`) toegevoegd. Roadmap, regressiegids en designnotitie bijgewerkt met undo/redo-stappen en focusverwachtingen.
   - **QA-notes** – Geautomatiseerd: `npm run lint`, `npm run test`, `npm run build` (Node 20.19.0). Handmatig: in popup en dashboard meerdere secties pinnen/ verbergen, toast met Undo/Redo volgen, focusbehoud controleren en bevestigen dat content-zijbalk dezelfde statuswijzigingen en toasts ontvangt (Chrome/Edge op `chat.openai.com` en `chatgpt.com`, LTR + RTL).
19. [x] Resetknop voor zijbalkvoorkeuren toevoegen aan popup, dashboard en content. _(afgerond 2025-10-19)_
   - **Prioritering** – Gebruikers kunnen nu met één klik alle pin-, verberg- en inklapvoorkeuren herstellen zonder handmatig elke toggle terug te zetten. De reset bouwt op de bestaande undo/redo-stack zodat vorige configuraties direct terughaalbaar blijven en vormt de springplank voor een latere "herstel verborgen secties"-flow.
   - **Documentatie** – `src/shared/state/sidebarVisibilityStore.ts` uitgebreid met `resetSections`, UI-knoppen toegevoegd in `src/popup/features/sidebar/SidebarSection.tsx` en `src/options/features/sidebar/SidebarPreferences.tsx`, toastfallbacks bijgewerkt (`src/shared/components/SidebarVisibilityToast.tsx`, i18n `src/shared/i18n/locales/{en,nl}/common.json`) en testdekking aangevuld (`tests/shared/state/sidebarVisibilityStore.spec.ts`). Roadmap (`docs/handbook/product-roadmap.md`) en regressiegids (`docs/handbook/manual-regression-checklist.md`) beschrijven nu de reset-flow.
   - **QA-notes** – Geautomatiseerd: `npm run lint`, `npm run test`, `npm run build` (Node 20.19.0). Handmatig: in popup, dashboard en content **Lay-out resetten** activeren, controleren dat alle secties terug naar de standaardstatus gaan, toastmelding "Zijbalkindeling hersteld" verschijnt en Undo/Redo de vorige voorkeuren terugzetten (Chrome/Edge, `chat.openai.com` + `chatgpt.com`, LTR + RTL).
20. [x] Basis-UI voor mapcreatie implementeren in de zijbalk. _(afgerond 2025-10-20)_
...
| 2025-10-17 | _pending_ | Accessibility | Collapse/pin/hide-bediening voorzien van aria-labels, regio-labeling en groepsbeschrijvingen (`src/content/ui-root.tsx`, `src/popup/features/sidebar/SidebarSection.tsx` + i18n). Roadmap, designnotitie, regressiegids en retrofitlog bijgewerkt; lint/test/build gerund en handmatig screenreaderlabels gevalideerd (EN/NL). |
