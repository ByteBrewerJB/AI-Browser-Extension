...
18. [ ] Undo/redo-flows voor bulk pin/hide-acties ontwerpen en implementeren. _(gepland)_
   - **Prioritering** – Met toegankelijke bediening op orde is de volgende iteratie het toevoegen van undo/redo-stacks voor bulkacties zodat toetsenbordgebruikers directe feedback + herstelopties krijgen.
   - **Documentatie** – Koppelen aan roadmapsectie "Search & sidebar" zodra scope gedetailleerd is; regressiegids voorbereiden met extra QA-stappen voor toasts en focusretour.
   - **QA-notes** – Te plannen (Vitest voor stacklogica + handmatige verificatie van toast focusfouten).
